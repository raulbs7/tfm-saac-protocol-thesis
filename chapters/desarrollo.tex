...
\begin{table}[h]
  ...
  3 & Recuperación complementaria       & Eco + PANNs (\texttt{audio\_events.py})   & OE1 \\ \hline
...
modelos estándar. En primer lugar, se asume un escenario de dos hablantes: la aprendiz
...
%%=============================================================
\section{Etapa 3: Recuperación complementaria de turnos comunicativos}
...
contiene 44 \ac{CT} anotados por la especialista. Para complementar la diarización se
emplean dos métodos adicionales que operan igualmente sobre el canal auditivo de la
sesión, y no sobre el canal de vídeo. Un análisis de la brecha entre ambas cifras
permite identificar dos categorías de turnos perdidos.
...
permite al especialista revisar con especial atención los \ac{CT} recuperados por este
método.
...
\begin{table}[h]
  ...
  \end{tabular}
  \end{center}
  \caption[Cobertura por canal de recuperación]{Análisis de cobertura acumulada de los
    tres canales de recuperación sobre la sesión 1 («¿Dónde están las hadas y la luna?»).
    El porcentaje se calcula respecto a los 44~\ac{CT} del estándar de referencia anotado
    por la especialista, con el único fin de caracterizar el techo estructural del canal de
    audio; no constituye una evaluación formal del sistema (pendiente como trabajo futuro).
    El techo teórico se estima como el porcentaje de \ac{CT} cuyo contenido está presente
    en la transcripción bruta de \emph{Whisper}, incluyendo los atribuidos al interlocutor.}
...
Para esta tarea se propone una arquitectura de generación aumentada por recuperación
(\ac{RAG}), que combina las ventajas de los modelos de lenguaje grandes (capacidad de
generar texto en lenguaje natural estructurado) con la necesidad de basar las
propuestas exclusivamente en la evidencia de la sesión (los \ac{CT} analizados), sin
recurrir a conocimiento externo que podría introducir alucinaciones. Los trabajos de
...
español, y se almacenan en un índice \emph{FAISS}~\cite{faiss_johnson2021} que permite la
...
referencia (\emph{kappa de Cohen} campo a campo) no ha podido completarse en el
plazo de este trabajo y queda como trabajo futuro.
...
que permitiría superar el techo del 47~\% del canal de audio. En segundo lugar, la
finalización del módulo \ac{RAG} y su evaluación cuantitativa frente a las anotaciones
expertas completaría la demostración del sistema extremo a extremo. En tercer lugar, la
...
orientar los esfuerzos de mejora hacia los módulos con mayor impacto clínico. Por
último, el \textbf{empaquetamiento del sistema como instalador autónomo} —que incluya
la descarga y configuración automática de los modelos y los entornos de ejecución, de
modo que el clínico pueda instalar y ejecutar el prototipo sin asistencia técnica— es
una condición necesaria para la realización de la demostración con profesionales de la
\emph{Fábrica de Palabras} y para cualquier evaluación de utilidad clínica percibida.
