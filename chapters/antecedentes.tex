...
Las herramientas consolidadas de análisis automático de muestras de lenguaje —como
\ac{SALT}, \ac{CLAN} o \ac{SUGAR}— han sido diseñadas fundamentalmente para habla oral
típica en inglés \cite{lsa_computer_programs}. Aunque \ac{SALT} dispone de bases de
datos normativas en español para determinados contextos comunicativos, estas no han sido
construidas ni validadas sobre hablantes de \ac{SAAC}, por lo que su aplicación al
lenguaje asistido en español introduce sesgos que infravaloran la competencia real del
usuario. En ningún caso contemplan los rasgos específicos del lenguaje producido
mediante pictogramas. Esta carencia no es trivial:
el lenguaje asistido difiere estructuralmente del habla espontánea en aspectos que afectan
directamente a la segmentación de enunciados, al etiquetado morfosintáctico y al cálculo
de métricas de diversidad léxica. Paralelamente, los modelos de \ac{PLN} disponibles en
español no han sido evaluados sobre corpus de lenguaje asistido, por lo que su aplicación
directa en este dominio entraña riesgos de error sistemático que deben conocerse antes de
adoptarlos. La ausencia de herramientas validadas para el \ac{PAALSS} en español justifica
así la necesidad de una revisión crítica de los trabajos existentes y de las decisiones
de diseño que de ella se derivan.

Este trabajo se sitúa en la intersección de cuatro ámbitos de investigación que raramente
convergen: la clínica del lenguaje asistido por \ac{SAAC}, el análisis automático de
muestras de lenguaje, el procesamiento morfosintáctico del español con escasos recursos y
los sistemas de generación aumentada por recuperación (\ac{RAG}) aplicados a documentación
clínica. Comprender qué ha alcanzado cada uno de estos campos de forma independiente, y
qué brechas permanecen abiertas en su intersección, es condición necesaria para justificar
las decisiones de diseño del sistema propuesto y para interpretar con rigor sus resultados.

Este capítulo revisa el estado del arte en los campos que confluyen en este trabajo.
En la Sección~\ref{sec:saac} se describe el marco clínico: los \textbf{\ac{SAAC}}, las
características lingüísticas del lenguaje que producen sus usuarios y el
\textbf{protocolo de Gloria Soto} (\ac{PAALSS}) que se pretende automatizar, con el objeto
de delimitar con precisión el problema que aborda el sistema. La Sección~\ref{sec:lsa_automatico}
analiza las iniciativas de \ac{LSA}, tanto las herramientas clásicas como las propuestas más
recientes basadas en \ac{PLN} y aprendizaje profundo, identificando los aspectos que no se
transfieren directamente al lenguaje asistido en español. Las secciones siguientes revisan
las técnicas de aprendizaje con pocos ejemplos en entornos clínicos, los sistemas de
generación aumentada por recuperación aplicados a protocolos médicos y los avances en
transcripción automática y diarización de hablantes, todos ellos componentes del
\emph{pipeline} propuesto en este trabajo.
...
el usuario durante un turno de conversación.  El protocolo organiza el registro de estos
turnos en una hoja de Registro (véase la Figura~\ref{fig:paalss_registro}), en la que el
especialista consigna, turno a turno, la transcripción del enunciado, la modalidad de
producción (comunicador electrónico, vocalización u otra) y las observaciones clínicas
pertinentes.  El protocolo manual requiere que un especialista en \ac{SAAC} transcriba la
sesión, segmente los enunciados, los codifique según las cuatro dimensiones y calcule las
métricas globales.  Este proceso puede extenderse varias horas por sesión, lo que
constituye la principal barrera para su uso sistemático en la práctica clínica y
educativa~\cite{lsa_computer_programs}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=1\textwidth]{figs/paalss_form.png}
  \caption[Hoja de Registro del formulario \ac{PAALSS}]{Hoja de Registro del
  formulario \ac{PAALSS}. Cada fila corresponde a un \ac{CT} de la aprendiz e
  incluye su transcripción, la modalidad de producción (comunicador electrónico,
  vocalización u otra) y el espacio para anotaciones clínicas. El sistema
  desarrollado en este trabajo pre-rellena automáticamente estos campos a partir
  del procesamiento del audio de la sesión. Fuente: Soto~\cite{soto2020protocolo}.}
  \label{fig:paalss_registro}
\end{figure}
...
Ninguno de los tres programas resulta adecuado para el lenguaje asistido por \ac{SAAC}
en este proyecto: sus bases de datos normativas se construyeron sobre habla oral de
niños con desarrollo típico, sin contemplar la ausencia de morfología en los
pictogramas, el componente gestual y multimodal ni el telegrafismo estructural propios
del lenguaje asistido~\cite{lsa_computer_programs}. Que \ac{SALT} disponga de bases de
datos normativas en español no resuelve este problema, pues dichas bases tampoco han
sido construidas sobre hablantes de \ac{SAAC}: la limitación determinante no es el
idioma, sino la ausencia de un modelo de referencia para el lenguaje asistido. Esta carencia es 
precisamente la que motivó el desarrollo del protocolo \ac{PAALSS} por parte de Gloria Soto.
...
Las dos herramientas de referencia para el análisis morfosintáctico del español son
\emph{spaCy} \cite{lsa_swiss_benchmark} y \emph{Stanza}. \emph{spaCy} ofrece modelos basados
en redes neuronales para el etiquetado \ac{POS}, el análisis de dependencias y el
reconocimiento de entidades; su modelo \texttt{es\_core\_news\_lg} combina vectores de
palabras preentrenados con una red convolucional de secuencias y constituye la implementación
de referencia para el español en tareas industriales y de investigación. \emph{Stanza},
desarrollado por el grupo de \ac{PLN} de Stanford, incorpora modelos preentrenados sobre los
corpus \emph{Universal Dependencies} en español y proporciona soporte nativo para lematización
y análisis de dependencias. Ambas herramientas están disponibles como bibliotecas \emph{Python} de
código abierto y no requieren infraestructura remota, lo que las hace compatibles con el
requisito de procesamiento local de este trabajo. La Figura~\ref{fig:spacy_pipeline} muestra
los componentes del \emph{pipeline} secuencial de \emph{spaCy} para el modelo
\texttt{es\_core\_news\_lg}, que es el empleado en este trabajo para el análisis morfológico,
sintáctico y léxico de los \ac{CT}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.45\textwidth]{figs/spacy_architecture.png}
  \caption[Arquitectura de \emph{spaCy}]{Arquitectura de la biblioteca \emph{spaCy}.
    El \emph{pipeline} secuencial transforma el texto de entrada en un objeto
    \texttt{Doc} anotado mediante componentes encadenados: tokenizador, \emph{tok2vec}
    (representación vectorial contextual compartida), morfologizador (\ac{POS} y rasgos
    morfológicos), analizador de dependencias, \ac{NER} y lematizador.
    Fuente: \emph{spaCy}~\cite{lsa_swiss_benchmark}.}
  \label{fig:spacy_pipeline}
\end{figure}

Su rendimiento, sin embargo, cae de forma sistemática al alejarse del texto escrito formal.
El estudio de Bonilla~\cite{spoken_spanish_pos} presenta COSER-PoS, un corpus anotado
manualmente sobre transcripciones de habla oral espontánea en español, y reporta que
\emph{spaCy} alcanza una exactitud de entre 0{,}90 y 0{,}93 sobre este corpus, frente al
0{,}98--0{,}99 que obtiene sobre \emph{AnCora}, el corpus de texto escrito estándar. Esta
caída de entre cinco y ocho puntos porcentuales se concentra en las categorías funcionales
(determinantes, preposiciones y morfemas verbales) y se debe principalmente a la mayor
frecuencia de estructuras incompletas y de vocabulario infrecuente en el habla espontánea.
Para el lenguaje asistido por \ac{SAAC} la degradación puede ser aún mayor: la ausencia de
morfología flexiva, el orden de palabras atípico y el vocabulario restringido al catálogo de
pictogramas generan distribuciones de \emph{tokens} que se alejan significativamente de los
corpus de entrenamiento de ambos modelos.

Esta limitación motiva la búsqueda de estrategias de adaptación al dominio. Para las categorías
morfológicas con criterios explícitos, es decir, distinción nombre/verbo en pictogramas de función
semántica inequívoca,, se emplean heurísticos basados en el vocabulario \ac{ARASAAC}; para las
categorías ambiguas dependientes del contexto conversacional, como la modalidad verbal o el
tiempo gramatical inferido a partir del enunciado del interlocutor, se adopta el método de
aprendizaje con pocos ejemplos sobre el modelo \emph{BETO}, descrito en la
Sección~\ref{sec:fewshot}.
...
\begin{itemize}
  \item \emph{Alta variabilidad inter e intrasujeto.} La pronunciación, el ritmo y el
  vocabulario activo varían considerablemente entre usuarios de diferentes edades, niveles
  de desarrollo y patologías, y también entre sesiones de un mismo usuario en función del
  contexto y del interlocutor. Esta variabilidad dificulta la generalización de los modelos
  de transcripción y de análisis lingüístico entrenados sobre muestras homogéneas, e impone
  la necesidad de validar el sistema sobre muestras propias del usuario para el que se
  despliega, sin dar por supuesto que los umbrales calibrados sobre un usuario se transferirán
  a otro~\cite{lsa_swiss_benchmark}.

  \item \emph{Estructuras agramaticales e incompletas.} La presencia frecuente de oraciones
  truncadas, cambios de tema a mitad de enunciado y construcciones no canónicas confunde los
  modelos de análisis \ac{POS} y de dependencias entrenados sobre \emph{corpus} de habla adulta
  formal. Algunos resultados muestran caídas de entre ocho y quince puntos porcentuales en la 
  exactitud del etiquetado morfosintáctico sobre habla infantil respecto a la habla adulta estándar, 
  con errores concentrados en las categorías funcionales (determinantes, preposiciones y morfemas 
  verbales) que son precisamente las más relevantes para el análisis del 
  protocolo \ac{PAALSS}\cite{lsa_swiss_benchmark}.

  \item \emph{Escasez de datos anotados.} La anotación de \emph{corpus} clínicos requiere
  conocimiento especializado que solo poseen logopedas con formación específica en el
  protocolo, lo que encarece y ralentiza la construcción de conjuntos de entrenamiento.
  Las restricciones de privacidad aplicables a datos de menores con discapacidad (en
  particular el \ac{RGPD} en el contexto europeo) limitan además la posibilidad de compartir
  \emph{corpus} entre instituciones. Esta escasez descarta el ajuste fino de modelos de gran escala
  y motiva el uso de las técnicas de aprendizaje con pocos ejemplos descritas en la
  Sección~\ref{sec:fewshot}, que permiten aprovechar modelos preentrenados con una
  supervisión adicional mínima~\cite{fewshot_clinical_nlp}.
\end{itemize}
...
En el contexto de los modelos de lenguaje basados en \emph{Transformer}
(véase la Figura~\ref{fig:transformer_arch}), pueden
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.35\textwidth]{figs/transformer_architecture.png}
  \caption[Arquitectura \emph{Transformer}]{Arquitectura \emph{Transformer} original~\cite{vaswani2017attention}. El bloque de la izquierda es el \textbf{codificador} (\emph{encoder}), compuesto por capas de atención
    multi-cabeza y redes hacia adelante con conexiones residuales; el bloque de la
    derecha es el \textbf{decodificador} (\emph{decoder}), que añade atención cruzada
    sobre la salida del codificador.}
...
La arquitectura de un sistema \ac{RAG} se organiza en dos fases principales, cuyo
flujo de información se ilustra en la Figura~\ref{fig:rag_flujo}. En la fase de
\emph{indexación}, el corpus de referencia (en este caso los enunciados de las sesiones)
se vectoriza mediante un modelo de \emph{embeddings} y se almacena en una base de datos
vectorial que permite la búsqueda eficiente por similitud semántica. En implementaciones
locales se emplea habitualmente \emph{FAISS} (\emph{Facebook AI Similarity Search}), una
biblioteca de código abierto optimizada para la búsqueda por vecinos más próximos en
espacios vectoriales de alta dimensión. En la fase de \emph{consulta y generación}, cada
campo del formulario actúa como una consulta: la etapa de recuperación (\emph{retrieval})
extrae los $k$ enunciados más similares mediante similitud coseno entre el
\emph{embedding} de la consulta y los almacenados en el índice. El \ac{LLM} recibe un
\emph{prompt} estructurado que incluye los enunciados recuperados y las métricas numéricas
de la sesión (TNW, NDW, \ac{LME}) como contexto, y genera la observación fundamentada
exclusivamente en ellos, con la instrucción explícita de señalar la ausencia de información
cuando el contexto recuperado no sea suficiente.

\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}[
    node distance = 0.6cm and 1.0cm,
    box/.style  = {rectangle, draw=gray!55, rounded corners=3pt,
                   fill=gray!8, minimum width=2.5cm, minimum height=0.8cm,
                   align=center, font=\small},
    data/.style = {box, fill=tema!15},
    arr/.style  = {-{Stealth[length=5pt]}, semithick},
  ]
  % Fila superior: indexación
  \node[data]              (cts) {CTs de la sesión};
  \node[box, right=of cts] (emb) {\texttt{nomic-embed-text}\\(\emph{Ollama})};
  \node[box, right=of emb] (idx) {Índice vectorial\\\emph{FAISS}};
  % Fila inferior: consulta y generación
  \node[data, below=1.5cm of cts] (q)   {Consulta por\\dimensión \ac{PAALSS}};
  \node[data, below=0.5cm  of q]  (met) {Métricas PAALSS};
  \node[box,  right=of q]         (ret) {\emph{Retrieval}\\top-$k$ CTs};
  \node[box,  right=of ret]       (llm) {\emph{Prompt} +\\\texttt{llama3}};
  \node[data, right=of llm]       (obs) {Observación\\clínica};
  % Flechas de indexación
  \draw[arr] (cts) -- (emb);
  \draw[arr] (emb) -- (idx);
  % Flechas de consulta y generación
  \draw[arr] (q)   -- (ret);
  \draw[arr] (ret) -- (llm);
  \draw[arr] (llm) -- (obs);
  \draw[arr, dashed] (idx.south) -- ++(0,-.5) -| (ret.north);
  \draw[arr] (met.east) -- ++(0.3,0) |- (llm.west);
  \end{tikzpicture}
  \caption[Flujo del sistema \acs{RAG}]{Flujo de información del sistema \ac{RAG}:
  indexación vectorial de los enunciados de la sesión (fila superior) y consulta por
  dimensión \ac{PAALSS} con generación de observaciones clínicas ancladas en los
  enunciados recuperados (fila inferior). La flecha discontinua indica la recuperación
  por similitud coseno entre el índice \emph{FAISS} y la consulta.}
  \label{fig:rag_flujo}
\end{figure}
...
investigación avance y se puedan desarrollar nuevas funcionalidades a futuro.
...
(véase la Figura~\ref{fig:whisper_arch}),
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\textwidth]{figs/whisper-architecture.png}
  \caption[Arquitectura de \emph{Whisper}]{Arquitectura del modelo \emph{Whisper}.
    El \textbf{codificador} procesa el espectrograma \emph{log-mel} del audio mediante
    dos capas convolucionales y bloques \emph{Transformer} con codificación posicional
    sinusoidal. El \textbf{decodificador} genera los \emph{tokens} de la transcripción
    de forma autorregresiva mediante atención cruzada sobre la salida del codificador.
    Los \emph{tokens} especiales de inicio (\texttt{SOT}), idioma (\texttt{EN}) y tarea
    (\texttt{TRANSCRIBE}) permiten controlar el idioma y la modalidad de salida.
    Fuente: Radford et al.~\cite{whisper_radford2023}.}
...
Diarization-Dependent Transformations}~\cite{dicow_diarization}
(véase la Figura~\ref{fig:dicow_arch}), evitando así el
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.6\textwidth]{figs/dicow_architecture.png}
  \caption[Arquitectura de \emph{DiCoW}]{Arquitectura de \emph{DiCoW}
    (\emph{Diarization-Conditioned Whisper})~\cite{dicow_diarization}.}
  \label{fig:dicow_arch}
\end{figure}

Dentro del ecosistema \emph{pyannote}, el módulo \textbf{\emph{pyannote-segmentation}}~\cite{pyannote_plaquet2023}
constituye el componente base que detecta los cambios de hablante a nivel de \emph{frame}.
Opera sobre ventanas de audio de 10~s con solapamiento del 50\% y produce segmentos
temporales etiquetados con un identificador de hablante, que la etapa posterior de
\emph{clustering} agrupa en identidades únicas a lo largo de la sesión. Es este módulo
el que se emplea en el presente trabajo como primer paso de la diarización, antes de la
asignación de identidades mediante \emph{clustering} jerárquico aglomerativo, y su
integración directa como librería \emph{Python} facilita el despliegue local sin infraestructura
de servidor, en consonancia con el requisito \ac{RGPD} de procesamiento local.

Un aspecto específico de las sesiones de comunicación asistida es la presencia de la voz
sintetizada (\ac{TTS}) del comunicador electrónico, que difiere acústicamente del habla
humana por su entonación uniforme y la ausencia de disfluencias. Sin embargo, los modelos
de diarización de propósito general tienden a asignar estos fragmentos al clúster del
interlocutor adulto, dado que ambos comparten características de habla adulta fluida.
Para recuperar estos enunciados del comunicador se emplea en este trabajo una estrategia
complementaria basada en la detección de eco: las palabras del comunicador son
habitualmente repetidas por el interlocutor en los segundos siguientes, lo que permite
confirmar su atribución comparando la transcripción cruda de \emph{Whisper} con los
segmentos diarizados del interlocutor. Esta estrategia se describe en detalle en el
...
\begin{table}[h]
  ...
  Validado sobre pocos usuarios y sin acceso al registro de los pictogramas del dipositivo \\ \hline
  \end{tabular}
  \end{center}
